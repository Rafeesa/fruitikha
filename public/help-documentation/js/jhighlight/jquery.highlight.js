...
(function ($) {
  $.fn.highlight = function (element_params) {
    ...
    return this.each(function () {
      ...
      $(code_container).wrap(
        '<div class="highlight' + code_lang_class + '"></div>'
      );
      ...
      if (params.source == true) {
        ...
        var tabs =
          '<ul class="tabs"><li class="code active">code</li><li class="source">source</li></ul>';
      ...
      $(code_container).replaceWith(
        '<pre class="' + code_class + '">' + code + '</pre>'
      );

      //zebra
      if (params.zebra == true) {
        $(highlight_container)
          .find('pre[class="' + code_class + '"] ' + params.list + ' li:even')
          .addClass('even');
      }
    });
  };
  ...
  $.highlightCode = {
    ...
    hightlight: function (code) {
      ...
      code = code
        //replace keywords
        .replace(
          /(var|function|typeof|new|return|if|for|in|while|break|do|continue|case|switch)([^a-z0-9\$_])/gi,
          '<span class="kwd">$1</span>$2'
        )
        //replace keywords
    ...
    hightlight_css: function (code) {
      ...
      var values =
        'absolute all attr auto baseline behind below black blink block blue bold bolder ' +
      ...
      var fonts =
        '[mM]onospace [tT]ahoma [vV]erdana [aA]rial [hH]elvetica [sS]ans-serif [sS]erif [cC]ourier New mono sans serif';
    ...
    hightlight_html: function (code) {
      code = code
        //replace attributes
        .replace(
          /\s+([a-zA-Z\-]{0,15})\=\"([-a-z0-9_ \/\.\#\:\=\;]{0,49})\"/gi,
          ' <span class="atn">$1</span>=<span class="atv">"$2"</span>'
        )
        //replace open tags
        .replace(
          /(&lt;)(\w{0,15})(\s+|&gt;|>)/gi,
