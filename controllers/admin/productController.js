...
const deleteSingleImage = async (req, res) => {
  try {
    ...
    if (!imageNameToServer || !productIdToServer) {
      return res
        .status(400)
        .send({ status: false, message: 'Invalid request data' });
    }
    ...
    res
      .status(500)
      .send({ status: false, message: 'Error deleting the image' });
  }
};
...
const addProductOffer = async (req, res) => {
  try {
    ...
    findProduct.salePrice =
      findProduct.price - Math.floor(findProduct.price * (percentage / 100));
